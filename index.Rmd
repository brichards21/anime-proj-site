...
anime_full_df$num_studios <- rowSums(!is.na(tst[, c("studio1", "studio2", "studio3",
                                   "studio4")]), na.rm = T)


anime_full_df <- anime_full_df %>%
  select(-c(studio3, studio4)) %>%
...
anime_full_df <- anime_full_df %>%
  select(-c(genres, start_date, end_date, duration)) 
...
$$Score ~ Episodes + Rating + Members + Duration + Action + Comedy + Drama + Historical +$$

$$Magic + School + Shounen + Slice\: of\: Life + Start Year + Num. \:Years\: Running$$


```{r}

# Fit the full model 
full.model <- lm(score ~., data = na.omit(anime_full_mod_df_use))
...
fin_mod <- lm(formula = score ~ episodes + rating + members + duration_mins + 
    ...
    slice_of_life + start_year + num_years_running, data = anime_full_mod_df_use)
...
These are the genres that are more likely to have a high score on MyAnimeList.net in order:
...
rf = randomForest(x= anime_full_df[, c(4:18, 20:40)],
                  y= anime_full_df[, "score"])
