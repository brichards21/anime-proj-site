...
ggplot(test_score_pred, aes(x = 1:nrow(test_score_pred), y = pred - score)) +
  geom_point() +
  ...
  ylim(-3, 3.5) +
  theme_economist() +
  annotate(geom="text", x = 1200, y = 3, label = "Max Error: 2.64",
           col = "red", size = 5) +
   annotate(geom="text", x = 1200, y = -2.2, label = "Min Error: -1.79",
           col = "red", size = 5)

ggplot(test_score_pred, aes(x = pred - score)) +
  geom_density(fill = ) +
