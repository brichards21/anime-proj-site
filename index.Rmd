...
+-----------------------------------+-----------------------------------------------------------------------------------------------+
| Score                             | average score of the anime given from all users in MyAnimelist database. Score out of 10 pts. |
+-----------------------------------+-----------------------------------------------------------------------------------------------+
...
Additionally, we downloaded the `anime_with_synopsis` and the `rating_complete` data

```{r, include=FALSE}

anime_full_df <- read_csv("data/anime_df_full.csv") %>%
  select(-c(total_mins, ...1)) 
anime_full_df$...1
```

In particular, this raw dataset contains the following information:

I first chose to reduce our analysis to only include anime with more than one episodes, so now one-shots or distinctive OVAs. We wanted to get information about proper anime series.

```{r}
# count unique anime
length(unique(anime_full_df$mal_id))

```

## Research Questions

1) Do the scores of these anime differ based on all users vs. users that have that anime marked as completed?

2) Can the synopsis of an anime be a good indicator of anime ranking?
